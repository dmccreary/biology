...
async function loadGraph() {
    try {
        ...
        initializeNetwork();  // removes loading-message via stabilization event
        buildLegend();
        updateStats();
        setupSearch();
        setupControls();
...
// The JSON must be reordered to match concept-taxonomy.md before installing.
...
function updateVisibility() {
    ...
    // Never call update() inside a forEach — each call triggers a full redraw.
    nodes.update(allNodes.map(node => ({
        id: node.id,
        hidden: !visibleGroups.has(node.group)
    })));

    // Batch update edges — edges use explicit IDs assigned at load time,
    // guaranteeing DataSet lookup always succeeds.
...
function updateStats(visibleNodeIds) {
    ...
    // nodesWithDeps is precomputed at load time — never recomputed here
